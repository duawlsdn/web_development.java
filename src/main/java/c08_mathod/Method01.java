...
/*
    Method란? = Java에서는 함수 개념과 동일.

    Method : 일련의 동작을 묶어서 정의한것. 입력값과 출력값이 존재(optional)

    함수와 메서드가 동일한 개념이라면 왜 용어를 분리하는가?
            -> 클래서 내부에 함수를 정의하게 되면 이를 메서드라고 칭함.
            -> 자바의 경우는 모든 함수들이 특정 클래스에 종속돼있기 때문에 전부다 메서드에 해당함.

    호출 형식
    클래스명.메서드명() -> 대문자 시작
    객체명.메서드명()   -> scanner.nextLin() , 소문자 시작

    메서드 정의 : 특정한 메서드를 클래스 내에 만드는 것
    메서드 호출(call) : main method가 있는 곳에서 클래스명.메서드명() 혹은,
                    객체명.메서드명()을 통해서 메서드를 호출 할 수 있음 - > 호출 = 실행
    자바에서의 메서드
    ...
    미리 입력(메서드정의)해놓고 나서, '사진을 찍는다'라는 명령어를 실행(메서드 호출)시
    자동으로 1)~5)까지의 명령을 순서대로 수행함
    -> 즉 메서드란 순차적그올 이루어지는 명령어들을 모아둔 것


    2. 메서드 관련 용어 정리
        1) 메서드 정의(define) : 사용자가 메서드를 새로 만드는 것을 의미
        2) 인수(argument) : 사용자가 메서드에 전달할 입력(input)을 의미
        3) 매개변수(parameter) : 인수를 받아서 저장하는 변수를 의미
        4) 반환값/출력값/리턴값(return) : 사용자가 메서드의 출력(output)을 의미
        5) 메서드 호출(call) : 만들어진 사용자가 메서드를 실제로 사용하는 것을 의미
 */
    // 1번 유형 -> [ x | x ] [입력값(parament) | 출력값(return)]
...
//        routine = scanner.nextLine(); // -> "no"이라고 입력했다면, 그것은 scanner.nextLine()의 return값
//        // String 자료형이 저장된 변수 routine을 call2의 argument로 사용
//        call2(routine);
        // 이하의 방법으로는 return result를 콘솔에서 확인 할 수 없음
//        call3();
        // 별찍기, 즉 return값을 출력하기 위해 출력하라는 명령어를 집어 넣어야만 함.
//        System.out.println(call3());
//        call4(2024,11,29);
//        System.out.println(call4(2024,11,29)); // 실행 방법 1 : 출력문 내에 바로 메서드 호출
        // 변수 선언 및 초기화 -> 메서드의 return값을 toDayDate에 대입
//        String toDayDate = call4(2024,11,29);
//        System.out.println(toDayDate);  // 실행방법 2 : 별도의 변수를 선언하여 메서드의 return값을 저장한 후, 변수를 통해 sout 실행

        System.out.print("당신이 태어난 년도는 언제입니까? 숫자만 쓰세요. >>>>");
        int year = scanner.nextInt();
        System.out.print("당신이 태어난 월은 언제입니까? 숫자만 쓰세요. >>>>");
        int month = scanner.nextInt();
        System.out.print("당신이 태어난 일은 언제입니까? 숫자만 쓰세요. >>>>");
